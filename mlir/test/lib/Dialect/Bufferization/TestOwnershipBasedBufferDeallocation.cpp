...
struct TestOwnershipBasedBufferDeallocationPass
  ...
  void runOnOperation() override {
    ...
    WalkResult result = getOperation()->walk([&](FunctionOpInterface funcOp) {
      ...
      if (failed(bufferization::deallocateBuffersOwnershipBased(
              funcOp, symbolTables, options)))
      ...
      bufferization::populateBufferizationDeallocLoweringPattern(
          memrefPatterns, deallocHelperFuncMap);
      ...
      options.getDeallocReplacement =
        ...
        if (auto gpuDealloc = dyn_cast<gpu::DeallocOp>(op)) {
          if (gpuDealloc.getAsyncToken()) {
            ...
            ValueRange token = gpu::WaitOp::create(builder, op->getLoc(),
                                                   gpu::AsyncTokenType::get(
                                                       builder.getContext()),
                                                   ValueRange{})
                                   .getResults();
            return token;
      ...
      loweringOptions.buildDeallocOp = [](OpBuilder &builder, Location loc,
        ...
        Value token =
            gpu::DeallocOp::create(
                builder, loc, gpu::AsyncTokenType::get(builder.getContext()),
                memref)
                .getResult(0);
        gpu::WaitOp::create(builder, loc, Type(), token);
      };
      bufferization::populateBufferizationDeallocLoweringPattern(
          gpuPatterns, deallocHelperFuncMap, loweringOptions);
      if (failed(
