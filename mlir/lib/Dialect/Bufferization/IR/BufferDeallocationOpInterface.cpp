...
Value DeallocationState::getMemrefWithUniqueOwnership(
  ...
  while (!worklist.empty()) {
    ...
    if (allKnown) {
      ...
      Value resultPtr = memref::ExtractAlignedPointerAsIndexOp::create(
          builder, defOp->getLoc(), curr);
      Value ownership = getOwnership(operands.front(), block).getIndicator();

      for (Value val : ArrayRef(operands).drop_front()) {
        Value operandPtr = memref::ExtractAlignedPointerAsIndexOp::create(
            builder, defOp->getLoc(), val);
        Value isSameBuffer = arith::CmpIOp::create(builder, defOp->getLoc(),
                                                   arith::CmpIPredicate::eq,
                                                   resultPtr, operandPtr);
        Value newOwnership = getOwnership(val, block).getIndicator();
        ownership = arith::SelectOp::create(
            builder, defOp->getLoc(), isSameBuffer, newOwnership, ownership);
      }
